...
class GCN_LSTM(nn.Module):
    ...
    def forward(self, x:torch.Tensor, edge_index_stock:torch.IntTensor, edge_index_supplies_to:torch.IntTensor, edge_index_supplies_from:torch.IntTensor) -> torch.Tensor:
        ...
        Args:
            ...
            edge_index (_type_): Tensor of dimension batch*2*edges
        Returns:
        ...
        total_edges    = edge_index.amax(dim=(1, 2)).item() + 1 #ensure this is int
        new_edge_index = edge_index.repeat(time, 1, 1).view(batch, time, 2, edges) + total_edges*torch.arange(total_edges).repeat_interleave(total_edges)
