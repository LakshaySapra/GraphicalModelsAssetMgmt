...
def get_sector_edges(sector_df,all_sectors,date,gvkey_to_idx_mapper,gvkey_universe):
...
def get_supply_chain_edges(supchain_df,date,gvkey_to_idx_mapper,gvkey_universe):
...
def get_hist_ret_df(weekly_hist_ret_df,date,gvkey_to_idx_mapper,permno_to_gvkey_mapper,gvkey_universe,permno_universe):
...
def get_weekly_ret_df(weekly_ret_df,date,gvkey_to_idx_mapper,permno_to_gvkey_mapper,gvkey_universe,permno_universe):
...
class GNNDataset(Dataset):
    def __init__(self, data_path='../data/'):
        ...
        self.permno_universe = pd.read_pickle('data/permno_universe.pkl')
    ...
    def __getitem__(self,idx):
        ...
        # Sector edges
        sector_edge_lst = get_sector_edges(self.sector_df,self.all_sectors,cur_date,get_permnogvkey_toidx_mapper,cur_universe)
        # Consumer and supplier edges
        c_edge_lst, s_edge_lst = get_supply_chain_edges(self.supchain_df,cur_date,get_permnogvkey_toidx_mapper,cur_universe)

        # Individual stock features
        cur_hist_ret_df = get_hist_ret_df(self.weekly_hist_ret_df,cur_date,get_permnogvkey_toidx_mapper,get_mapper,cur_universe,cur_permno_universe)
        # Labels
        cur_weekly_ret_df = get_weekly_ret_df(self.weekly_ret_df,next_date,get_permnogvkey_toidx_mapper,get_mapper,cur_universe,cur_permno_universe)
